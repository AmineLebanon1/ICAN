...
modelF= keras.models.load_model('rec1.h5')
...
class OpenCamera (VideoProcessorBase):
    def __init__(self) -> None :
        ...
        self.sentence = []
        self.threshold = 0.4
        self.actions = np.array(['Weather', 'Cloud', 'Rain', 'Thunderstorm', 'Sunny', 'Snow'])
    ...
    def recv(self, frame):
        ...
        with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
            ...
            if len(self.sequence) == 30:
                ...
                # 3. Viz logic and output generation
                if res[np.argmax(res)] > self.threshold:
                    # Check if accuracy is greater than threshold
                    if res[np.argmax(res)] >= 0.5:  # Adjust the threshold as needed
                ...
                # Output only if hands are detected and accuracy is above threshold
                if keypoints.any():  # Check if keypoints (hands) are detected
                ...
                # If keypoints are not detected, clear the sentence
                self.sentence = []

        return av.VideoFrame.from_ndarray(image, format="bgr24")


st.sidebar.title('ICAN - INCLUSIVE COMMUNICATION FOR ARABIC NAVIGATION')

app_mode = st.sidebar.selectbox('App Mode', ('Demo', 'Live'))
